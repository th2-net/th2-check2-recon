...
class Rule:
    def __init__(self, event_store: store.Store, routing_keys: list, cache_size: int, time_interval: int) -> None:
        self.cache = services.Cache(cache_size, time_interval, routing_keys, event_store)
    ...
    @abstractmethod
    def check(self, messages: dict) -> infra_pb2.Event:
        pass
        ...
        for key in self.routing_keys:
            if self.cache.contains(hash_of_message, key):
                if key != routing_key:
                    ...
                    event_message = f"The message with the hash={hash_of_message} already contains in cache of {key}.  \
                        The implementation of the hash function is incorrect, as it allows collisions."
                    logger.debug(event_message)
                    self.event_store.store_error(routing_key, hash_of_message, message, event_message)
            else:
                break
    ...
    def check_and_store_event(self, messages_by_key: dict):
        ...
        for message in messages_by_key.values():
            ...
            if timestamp < self.cache.min_time or timestamp > self.cache.min_time + self.cache.time_interval:
                ...
                break
        if out_of_timeout:
            self.event_store.store_matched_out_of_timeout(check_event, self.cache.min_time,
                                                          self.cache.min_time + self.cache.time_interval)
        else:
            self.event_store.store_matched(check_event)
